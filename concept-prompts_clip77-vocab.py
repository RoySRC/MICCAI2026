...
ARTIFACT_PACK = ExpertPack(
    name="artifact",
    positives=[
        "blur with smeared texture and loss of crisp cellular edges.",
...
MVP_PACK = ExpertPack(
    ...
    positives=[
        "bundle clusters of small vessels forming rounded vascular aggregates.",
        "glomeruloid vascular bundles composed of crowded capillary loops.",
        "multiple closely packed vascular lumina within a compact vascular cluster.",
        "abnormally dense microvasculature with complex, bundle vessel profiles.",
        "proliferative capillary bundles with piled-up endothelial nuclei around small lumina.",
        "tangled, multilayered-appearing microvessels forming a compact vascular nidus.",
        "irregular, crowded capillary networks with rounded bundle-like vascular formations.",
        "microvascular hyperplasia with clustered capillary loops and thickened vascular profiles.",
    ],
    positives_short=[
        "glomeruloid vascular bundles",
        "bundle proliferative capillary loops",
...
NECROSIS_PACK = ExpertPack(
    ...
    hard_negatives=[
        "compact rounded clusters of many small circular lumina tightly grouped together.",
        "dense networks of tiny vessel-like loops forming a tight, bundle-like cluster.",
...
MICROCYSTS_PACK = ExpertPack(
    ...
    hard_negatives=[
        ...
        "repeated small looped ring-like profiles forming a tight bundle-like cluster within a focal region.",
...
REACTIVE_GLIOSIS_PACK = ExpertPack(
    ...
    hard_negatives=[
        ...
        # Competing morphology (vascular-bundle-like) without naming the hallmark
...
ARTIFACT_BLACKLIST = [
    ...
    "glomeruloid", "bundle", "bundle", "capillary bundle",
...
HIGH_VASCULARITY_BLACKLIST = [
    ...
    "glomeruloid",
    "bundle", "bundle", "capillary bundle",
    "endothelial", "endothelial proliferation", "endothelial hyperplasia",
    "angiogenesis", "neovascular", "vascular hyperplasia",
...
# =============================================================================
# 3.5) Optional vocabulary constraint for MODEL-generated prompts
# =============================================================================

_WORD_RE = re.compile(r"[A-Za-z]+(?:'[A-Za-z]+)?")


def load_vocabulary_txt(path: str) -> List[str]:
    """Load allowed WordPiece tokens from a TXT file.

    Format:
    - One token per line, or whitespace-separated tokens.
    - IMPORTANT: tokens may legitimately start with '##' (WordPiece continuation).
    - Optional comments are supported only when they start with '# ' or '//' at line start,
      or when preceded by whitespace ' #' (to avoid clobbering '##' tokens).
    ...
    if not os.path.exists(path):
        raise FileNotFoundError(f"vocab_txt not found: {path}")
    ...
    with open(path, "r", encoding="utf-8") as f:
        for raw in f:
            ...
            # Full-line comments (do NOT treat '##' as comment)
            if line.startswith("# ") or line.startswith("//"):
                continue

            # Inline comments only if preceded by whitespace to preserve '##xxx' tokens
            if " #" in line:
...
def _wordpiece_tokenizable(word_lower: str, vocab_lower: set) -> bool:
    """Return True if word_lower can be segmented into vocab_lower WordPiece tokens.

    Rule enforced:
    - First piece is matched without '##'
    - All subsequent pieces must be matched with '##' prefix
    """
    if not word_lower:
        return True

    # Fast path: exact token exists (for whole-word tokens like "rounded" if present)
    if word_lower in vocab_lower:
        return True
    ...
    while start < n:
        ...
        while end > start:
            piece = word_lower[start:end]
            if start > 0:
                piece = "##" + piece
            if piece in vocab_lower:
                matched_end = end
                break
            end -= 1
...
def vocab_oov_words(text: str, allowed_vocab_lower: set) -> List[str]:
    """Return out-of-vocabulary words, where "in-vocab" means WordPiece-tokenizable.

    Example:
    - 'rounded' is accepted if it can be segmented as 'round' + '##ed'
      (or any other valid WordPiece segmentation).
    """
    if not allowed_vocab_lower:
        return []
    ...
    for w in _extract_words(text):
        ...
        if _wordpiece_tokenizable(wl, allowed_vocab_lower):
            continue
...
def _extract_words(text: str) -> List[str]:
    """Extract alphabetic word tokens used for vocabulary enforcement.

    Notes:
    - Punctuation is ignored.
    - Words are compared case-insensitively.
    - Hyphenated chunks are split implicitly by the regex.
    """
...
- If allowed_vocabulary is provided in the input, you MUST use ONLY words from that list (case-insensitive).
  - You may use punctuation and numbers.
  - Do not use any alphabetic word that is not present in allowed_vocabulary.
...
def generate_variants_with_gpt(
    ...
    allowed_vocabulary: Optional[List[str]] = None,
    vocab_send_limit: int = 40000000000,
    max_rounds: int = 6,
) -> Dict[str, List[str]]:
    """
    ...
    allowed_vocab_lower: Optional[set] = None
    vocab_for_model: Optional[List[str]] = None
    if allowed_vocabulary:
        allowed_vocab_lower = {w.lower() for w in allowed_vocabulary if str(w).strip()}
        if len(allowed_vocabulary) <= vocab_send_limit:
            vocab_for_model = allowed_vocabulary
        else:
            # Avoid exploding the prompt/context. We'll still *enforce* the vocabulary locally.
            vocab_for_model = None

    tools = []
    if vector_store_id:
        tools.append({"type": "file_search", "vector_store_ids": [vector_store_id]})

    positives_out: List[str] = []
    hard_negatives_out: List[str] = []
    oov_counter: Dict[str, int] = {}

    def _accept_with_vocab_filter(kind: str, items: List[str]) -> List[str]:
        if not allowed_vocab_lower:
            return items
        kept: List[str] = []
        for s in items:
            oov = vocab_oov_words(s, allowed_vocab_lower)
            if oov:
                for w in oov:
                    wl = w.lower()
                    oov_counter[wl] = oov_counter.get(wl, 0) + 1
                continue
            kept.append(s)
        return kept
    ...
    for round_idx in range(max_rounds):
        rem_pos = max(0, need_pos - len(positives_out))
        rem_neg = max(0, need_neg - len(hard_negatives_out))
        if rem_pos == 0 and rem_neg == 0:
            break

        payload = {
            "expert": expert_name,
            "counts": {"positives": rem_pos, "hard_negatives": rem_neg},
            ...
            "instructions": [
                ...
                "Avoid duplicates of already accepted prompts.",
            ],
            "already_accepted": {
                "positives": positives_out[-16:],
                "hard_negatives": hard_negatives_out[-16:],
            },
        }

        if vocab_for_model is not None:
            payload["allowed_vocabulary"] = vocab_for_model
            payload["allowed_vocabulary_policy"] = (
                "All alphabetic words in every prompt must come from allowed_vocabulary (case-insensitive). "
                "Punctuation and numbers are allowed."
            )
        elif allowed_vocabulary:
            payload["allowed_vocabulary_policy"] = (
                "A vocabulary is enforced locally, but the full list is not included due to size. "
                "Use only common words already seen in the seed_examples and avoid introducing new wording."
            )

        if oov_counter:
            worst = sorted(oov_counter.items(), key=lambda kv: kv[1], reverse=True)[:25]
            payload["recent_oov_words_to_avoid"] = [w for w, _ in worst]


        PROMPT_VARIANTS_JSON_SCHEMA = {
            "type": "object",
            "properties": {
                "expert": {"type": "string"},
            ...
            "required": ["expert", "positives", "hard_negatives"],
        ...
        resp = client.responses.create(
            ...
            text={
                "format": {
                    "type": "json_schema",
                    "name": "expert_prompt_variants",
                    "schema": PROMPT_VARIANTS_JSON_SCHEMA,
                    "strict": True, 
                }
            },
        )
        ...
        pos_raw = [str(x).strip() for x in data.get("positives", []) if str(x).strip()]
        neg_raw = [str(x).strip() for x in data.get("hard_negatives", []) if str(x).strip()]

        pos_kept = _accept_with_vocab_filter("positives", pos_raw)
        neg_kept = _accept_with_vocab_filter("hard_negatives", neg_raw)

        # Preserve order, avoid duplicates.
        for p in pos_kept:
            if p not in positives_out:
                positives_out.append(p)
        for p in neg_kept:
            if p not in hard_negatives_out:
                hard_negatives_out.append(p)

        if round_idx == 0:
            print(f"{data = }")

    # if len(positives_out) < need_pos or len(hard_negatives_out) < need_neg:
    #     msg = [
    #         f"Vocabulary-constrained generation did not reach requested counts for expert '{expert_name}'.",
    #         f"Requested: positives={need_pos}, hard_negatives={need_neg}.",
    #         f"Got: positives={len(positives_out)}, hard_negatives={len(hard_negatives_out)}.",
    #     ]
    #     if oov_counter:
    #         worst = sorted(oov_counter.items(), key=lambda kv: kv[1], reverse=True)[:20]
    #         msg.append("Most frequent out-of-vocabulary words (lowercased): " + ", ".join([w for w, _ in worst]))
    #     raise ValueError("\n".join(msg))

    return {
        "positives": positives_out[:need_pos],
        "hard_negatives": hard_negatives_out[:need_neg],
    }
...
def repair_prompt_with_gpt(
    ...
    if allowed_vocabulary:
        ...
        if len(allowed_vocabulary) <= vocab_send_limit:
            ...
            print(f"{len(allowed_vocabulary) = }")
    ...
    repair_spec = {
        ...
        "allowed_vocabulary": vocab_for_model,
        "allowed_vocabulary_policy": (
            "If allowed_vocabulary is provided, all alphabetic words in the rewritten prompt must come from it (case-insensitive). "
            "Punctuation and numbers are allowed."
        ),
        "global_blacklist": GLOBAL_BLACKLIST,
    ...
    for violation in violations:
        if violation.startswith("vocab_oov:"):
            # extract the list portion and safely evaluate it
            oov_words = ast.literal_eval(violation.split(":", 1)[1])
            for word in oov_words:
                repair_spec['rewrite_rules'].append(
                    f"Substitute the word `{word}` with the most similar word from allowed_vocabulary."
                )
...
def lint_and_repair_bank(
    ...
    allowed_vocabulary: Optional[List[str]] = None,
    vocab_send_limit: int = 400000000000,
    ...
    for p in positives:
        ...
        if allowed_vocab_lower:
            oov = vocab_oov_words(p0, allowed_vocab_lower)
            if oov:
                violations.append(f"vocab_oov:{oov}")
        if violations and allow_gpt_repair:
            ...
            for _ in range(max_repair_rounds):
                cur = repair_prompt_with_gpt(
                    expert_name,
                    cur,
                    "positive",
                    violations,
                    allowed_vocabulary=allowed_vocabulary,
                    vocab_send_limit=vocab_send_limit,
                )
                violations = lint_prompt(expert_name, cur, "positive")
                if allowed_vocab_lower:
                    oov = vocab_oov_words(cur, allowed_vocab_lower)
                    if oov:
                        violations.append(f"vocab_oov:{oov}")
                if not violations:
                    break
...
def build_prompt_pack(
    ...
    vocab_scope: str = "gpt_only",  # "gpt_only" or "all"
    vocab_send_limit: int = 40000000000,
    ...
    """
    if vocab_scope not in {"gpt_only", "all"}:
        raise ValueError("vocab_scope must be one of: 'gpt_only', 'all'")

    allowed_vocabulary: Optional[List[str]] = None
    if vocab_txt:
        allowed_vocabulary = load_vocabulary_txt(vocab_txt)
        if not allowed_vocabulary:
            raise ValueError(f"vocab_txt provided but no words were loaded: {vocab_txt}")
    ...
    for expert_name, expert in EXPERTS.items():
        ...
        # Optional: expand with GPT (vocabulary-constrained if vocab_txt is provided)
        pos_extra: List[str] = []
        neg_extra: List[str] = []
        ...
        # Lint seeds (optionally enforce vocab for *all* prompts)
        seed_vocab = allowed_vocabulary if (allowed_vocabulary and vocab_scope == "all") else None
        pos_seed_fixed, neg_seed_fixed = lint_and_repair_bank(
            expert_name=expert_name,
            positives=pos_seed,
            hard_negatives=neg_seed,
            allow_gpt_repair=True,
            allowed_vocabulary=seed_vocab,
            vocab_send_limit=vocab_send_limit,
        )

        # Lint + (optional) repair extras (always enforce vocab if provided; these are model-generated)
        pos_extra_fixed: List[str] = []
        neg_extra_fixed: List[str] = []
        if pos_extra or neg_extra:
            pos_extra_fixed, neg_extra_fixed = lint_and_repair_bank(
                expert_name=expert_name,
                positives=pos_extra,
                hard_negatives=neg_extra,
                allow_gpt_repair=True,
                allowed_vocabulary=allowed_vocabulary,
                vocab_send_limit=vocab_send_limit,
            )

        positives_fixed = _dedupe_preserve_order(pos_seed_fixed + pos_extra_fixed)
        hard_negatives_fixed = _dedupe_preserve_order(neg_seed_fixed + neg_extra_fixed)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Build an HSI concept prompt pack (seed + optional GPT expansion), with strict linting. "
            "Optionally constrain MODEL-generated prompts to a vocabulary.TXT wordlist."
        )
    )
    parser.add_argument("--out", type=str, default="hsi_concept_prompt_pack.json", help="Output JSON path")
    parser.add_argument("--no_gpt", action="store_true", help="Disable GPT expansion; export seed-only pack")
    parser.add_argument("--extra_pos", type=int, default=64, help="Extra positive prompts per expert (GPT)")
    parser.add_argument("--extra_neg", type=int, default=64, help="Extra hard-negative prompts per expert (GPT)")
    parser.add_argument(
        "--vector_store_id",
        type=str,
        default="vs_6980fdd441688191ac026b6c0c69964e",
        help="Optional OpenAI vector store id for file_search grounding (set empty to disable)",
    )
    parser.add_argument("--skip_audit", action="store_true", help="Skip the pre-export audit")
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to vocabulary.TXT. If provided, MODEL-generated prompts must use only these words.",
    )
    parser.add_argument(
        "--vocab_scope",
        type=str,
        choices=["gpt_only", "all"],
        default="gpt_only",
        help=(
            "Apply vocab constraint only to MODEL-generated prompts ('gpt_only') or to all prompts ('all'). "
            "If 'all' and seeds contain out-of-vocab words, the run will fail."
        ),
    )
    parser.add_argument(
        "--vocab_send_limit",
        ...
        default=4000,
        help=(
            "Max number of vocabulary words to include in the model input. "
            "If the vocab is larger, it will be enforced locally via filtering, but not fully shown to the model."
        ),
    )
    ...
    vector_store_id = args.vector_store_id.strip() if args.vector_store_id else None
    if vector_store_id == "":
        vector_store_id = None

    prompt_pack = build_prompt_pack(
        expand_with_gpt=(not args.no_gpt),
        extra_pos_per_expert=int(args.extra_pos),
        extra_neg_per_expert=int(args.extra_neg),
        vector_store_id=vector_store_id,
        vocab_txt=args.vocab_txt,
        vocab_scope=args.vocab_scope,
        vocab_send_limit=int(args.vocab_send_limit),
    )
    save_prompt_pack(prompt_pack, args.out)
